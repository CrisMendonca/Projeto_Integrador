            ...
            steps { 
                script {
                    if(env.GIT_BRANCH=='origin/homologacao'){
                        ...
                        sh "docker run -d --name lovelace -p 8030:3000 -e NODE_ENV=homologacao -e AWS_ACCESS_KEY=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e BUCKET_NAME=digitalhouse-grupolovelace-homologacao 185721683284.dkr.ecr.us-east-1.amazonaws.com/lovelace:latest"
                        }
